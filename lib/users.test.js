...
describe("users", () => {
  ...
  context("reset password", () => {
    it("should reject if no email", async () => {
      expect(users.resetPassword()).to.eventually.be.rejectedWith(
        "Invalid email"
      );
    });

    it("should call sendPasswordResetEmail() with mailer", async () => {
      const resetPasswordStub = sandbox
        .stub(mailer, "sendPasswordResetEmail")
        .resolves("reset");

      await users.resetPassword("user@example.com");

      expect(resetPasswordStub).to.have.been.calledWith("user@example.com");
    });
  });
});
